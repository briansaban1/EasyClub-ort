...
export const getActividad = (actividadId) => {
  return async (dispatch) => {
    dispatch({ type: ActionTypes.GET_ACTIVIDAD_START });
      const data = await wservice.getActividad(actividadId)
      if (data.status == 1){
      dispatch({ type: ActionTypes.GET_ACTIVIDAD_SUCCESS, payload: data.data });
       } 
      dispatch({ type: ActionTypes.GET_ACTIVIDAD_FAILED });
  };
};
